...
let params = {BladderWidth:80 ,CervixWidth:29};
const maxUndos = 75;
...
> = redo
k = delete block
load(loadString) = when entered in console, loadString will be loaded
=======================================================================
...
function splitCurve(curve, start, end){
    ...
    let startSplitCurve = {
        ...
        x2: lerp(lerp(curve.x1, curve.x2, start),lerp(curve.x2, curve.x3, start),start),
        y2: lerp(lerp(curve.y1, curve.y2, start),lerp(curve.y2, curve.y3, start),start),
        x3: lerp(curve.x2, curve.x3, start),
        y3: lerp(curve.y2, curve.y3, start),
        x4: curve.x4,
        y4: curve.y4
    };
    return {
        ...
        x2: lerp(startSplitCurve.x1, startSplitCurve.x2, end - start),
        y2: lerp(startSplitCurve.y1, startSplitCurve.y2, end - start),
        x3: lerp(lerp(curve.x1, curve.x2, end - start),lerp(curve.x2, curve.x3, end - start),end - start),
        y3: lerp(lerp(curve.y1, curve.y2, end - start),lerp(curve.y2, curve.y3, end - start),end - start),
        x4: evalSpline(curve, end - start).x,
        y4: evalSpline(curve, end - start).y
    };
}
