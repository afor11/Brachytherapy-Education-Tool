...
function initModule(mod){
    ...
    if (mod === "planar array of seeds"){
        let tick = [];
        for (let j = -10; j <= 10; j+= 0.25){tick.push(j);}
        moduleData = {
        ...
        for (let i = -1.5; i <= 1.5; i += 1){
            for (let j = -1.5; j <= 1.55; j += 1){
                moduleData.graph1.seeds.push(new Seed({x: j, y: i, z: 0},{roll: 0, pitch: 0, yaw: 0},Best2301,airKermaSliderLimits.LDR.min,0.00833));
                moduleData.graph2.seeds.push(new Seed({x: j, y: i, z: 0},{roll: 0, pitch: 0, yaw: 0},Best2301,airKermaSliderLimits.LDR.min,0.00833));
            }
        }
    }
