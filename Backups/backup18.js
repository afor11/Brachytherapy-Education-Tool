...
function tick(){
    if ((canvas.width != window.innerWidth) || (canvas.height != window.innerHeight)){
        ...
        if (render){
            if (module === "brachytherapy applicators"){
                if (resetGraphs){
                    adjustApplicatorGraphFormat(moduleData.graph1,moduleData.applicatorModel,{refPoints: "graph1refPoints", drawFunction: "graph1DrawApplicator", boundingBox: moduleData.divBoundGraph1, view: ((moduleData.applicatorModel.name === "Tandem/Ring") ? "side" : "front")});
    ...
    if (module === "brachytherapy applicators"){
        ...
        adjustApplicatorRenderer(moduleData.graph1,moduleData.applicatorModel,{refPoints: "graph1refPoints", drawFunction: "graph1DrawApplicator", boundingBox: moduleData.divBoundGraph1, view: ((moduleData.applicatorModel.name === "Tandem/Ring") ? "side" : "front")});
        ...
        if (numGraphs > 1){
            for (let i = 0; i < numGraphs; i++){
                for (let k = 0; k < numGraphs; k++){
                    /*
                    Transforms (minXtick,minYtick,zSlice of graph k) from the of perspective of graph k to the actual
                    point in space then transforms that new point based on the perspective of graph i. For example
                    if i = 1 and k = 3 then this new point would be (minXtick,zSlice of graph k,minYtick). Notice that
                    this perspective will swap zSlice with the x or y position, assuming i != k, which we can use to
                    draw the line from the screenPos of this point to the screenPos of a similarly transformed point
                    */
                    if (i != k){
                        let barPos1 = graphToScreenPos(
                            moduleData[`graph${i + 1}`].perspective(
                                moduleData[`graph${k + 1}`].perspective({
                                    x: getMin(moduleData[`graph${i + 1}`].xTicks),
                                    y: getMin(moduleData[`graph${i + 1}`].yTicks),
                                    z: moduleData[`graph${k + 1}`].zSlice
                                })
                            ),
                            moduleData[`graph${i + 1}`],
                            moduleData[`divBoundGraph${i + 1}`]
                        );
                        let barPos2 = graphToScreenPos(
                            moduleData[`graph${i + 1}`].perspective(
                                moduleData[`graph${k + 1}`].perspective({
                                    x: getMax(moduleData[`graph${i + 1}`].xTicks),
                                    y: getMax(moduleData[`graph${i + 1}`].yTicks),
                                    z: moduleData[`graph${k + 1}`].zSlice
                                })
                            ),
                            moduleData[`graph${i + 1}`],
                        ...
                        ctx.beginPath();
                        ctx.moveTo(barPos1.x,barPos1.y);
                        ctx.lineTo(barPos2.x,barPos2.y);
                        ctx.stroke();
                    }
                }
            }
