...
function tick(){
    ...
    if (module === "brachytherapy applicators"){
        ...
        //draw treatment time
        let treatmentTime = "Total Treatment Time: " + (Math.round((moduleData.graph1.seeds.reduce((acc,curr) => acc + curr.dwellTime, 0) * 60) * 100) / 100) + " mins";
        ...
        if (moduleData.applicatorModel.name === "Vaginal Cylinder"){
            ...
            [{x: 2,y: 2.2,z: 0},{x: -2,y: 2.2,z: 0}].forEach((refpoint) => {
                let adjustedRefpoint = moduleData.graph1.perspective(refpoint);
                // draw graph1 point a's
                ctx.beginPath();
                let refPos = graphToScreenPos(adjustedRefpoint,moduleData.graph1,moduleData.divBoundGraph1);
                ctx.arc(refPos.x,refPos.y,Math.min(moduleData.divBoundGraph1.width,moduleData.divBoundGraph1.height) * 0.01, 0, 7);
                ctx.fill();
                let initalZ = moduleData.graph1.zSlice;
                moduleData.graph1.zSlice = adjustedRefpoint.z; // adjust zSlice of graph so dose is calculated at correct depth
                ctx.fillText(moduleData.graph1.getPointDose(adjustedRefpoint).toFixed(2),refPos.x + Math.min(moduleData.divBoundGraph1.width,moduleData.divBoundGraph1.height) * 0.02,refPos.y);
                moduleData.graph1.zSlice = initalZ; // reset zSlice

                adjustedRefpoint = moduleData.graph2.perspective(adjustedRefpoint);
                // draw graph2 point a's
                ctx.beginPath();
                refPos = graphToScreenPos(adjustedRefpoint,moduleData.graph2,moduleData.divBoundGraph2);
                ctx.arc(refPos.x,refPos.y,Math.min(moduleData.divBoundGraph2.width,moduleData.divBoundGraph2.height) * 0.01, 0, 7);
                ctx.fill();
                initalZ = moduleData.graph2.zSlice;
                moduleData.graph2.zSlice = adjustedRefpoint.z; // adjust zSlice of graph so dose is calculated at correct depth
                ctx.fillText(moduleData.graph2.getPointDose(adjustedRefpoint).toFixed(2),refPos.x + Math.min(moduleData.divBoundGraph2.width,moduleData.divBoundGraph2.height) * 0.02,refPos.y);
                moduleData.graph2.zSlice = initalZ; // reset zSlice
            });
        }
    }
    ...
    if (moduleData.graph2 && ((module === "brachytherapy applicators") ? !(moduleData.applicatorModel.name === "Vaginal Cylinder") : true)){
        if (!(module === "brachytherapy applicators")){
