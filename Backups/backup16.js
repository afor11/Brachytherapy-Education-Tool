...
function tick(){
    ...
    if (module != "single seed"){
        ...
        for (let i = 0; i < numGraphs; i++){
            ...
            let refPos = graphToScreenPos(moduleData[`graph${i + 1}refPoint`],moduleData[`graph${i + 1}`],divBound);
            ctx.fillStyle = "black";
            ctx.beginPath();
            ctx.arc(refPos.x,refPos.y,Math.min(divBound.width,divBound.height) * 0.01,0,2 * Math.PI);
            ctx.fill();

            moduleData[`graph${i + 1}refPointLabel`].draw();
        }
    }
...
function initModule(mod,previousMod){
    ...
    if (mod != "single seed"){
        ...
        if (mod === "brachytherapy applicators"){
            numGraphs = 3;
            dwellTimeSliders = [moduleData.graphDwellTimeSlider,moduleData.graphDwellTimeSlider,moduleData.graphDwellTimeSlider];
            airKermaSliders = [moduleData.graphAirKermaSlider,moduleData.graphAirKermaSlider,moduleData.graphAirKermaSlider];
        }
        for (let i = 0; i < numGraphs; i++){
            moduleData[`graph${i + 1}refPointLabel`] = new NumberInput({
                x: 0, y: 0, width: 0, height: 0,
                label: {
                    text: (value) => `${value}Gy`,
                    color: {selected: "white", notSelected: "black"}
                },bgColor: {selected: "black", notSelected: "white"},
                getValue: () => moduleData[`graph${i + 1}`].getPointDose(moduleData[`graph${i + 1}refPoint`]),
                onEnter: function (value){
                    if (mod === "brachytherapy applicators"){
                        let numReferencePoints;
                        if (moduleData.applicatorModel.name === "Vaginal Cylinder"){numReferencePoints = 1;}
                        if (moduleData.applicatorModel.name === "Tandem/Ovoids"){numReferencePoints = 3;}
                        if (moduleData.applicatorModel.name === "Tandem/Ring"){numReferencePoints = 2;}
                        for (let j = 0; j < numReferencePoints; j++){
                            setDoseAtPoint(moduleData[`graph${j + 1}`],moduleData[`graph${j + 1}refPoint`],value,dwellTimeSliders[j],airKermaSliders[j]);
                        }
                    }else{
                        setDoseAtPoint(moduleData[`graph${i + 1}`],moduleData[`graph${i + 1}refPoint`],value,dwellTimeSliders[i],airKermaSliders[i]);
                    }
                },
                numDecimalsEditing: 3
            });
...
function adjustFormatting(){
    ...
    if (module != "single seed"){
        ...
        if (module === "string of seeds"){numGraphs = 1}
        if (module === "planar array of seeds"){numGraphs = 2}
        if (module === "brachytherapy applicators"){numGraphs = 3}
        for (let i = 0; i < numGraphs; i++){
            let refPointLabel = moduleData[`graph${i + 1}refPointLabel`];
            let divBound = moduleData[`divBoundGraph${i + 1}`];
            let refPos = graphToScreenPos(moduleData[`graph${i + 1}refPoint`],moduleData[`graph${i + 1}`],divBound);
...
function checkOnMouseDown(){ // run only on mouse down
    ...
    if (module != "single seed"){
        ...
        if (module === "brachytherapy applicators"){numGraphs = 3}
        for (let i = 0; i < numGraphs; i++){
            moduleData[`graph${i + 1}refPointLabel`].checkClicked();
        }
    }
}
...
addEventListener("keydown",function (e) {
    ...
    if (module != "single seed"){
        ...
        if (module === "brachytherapy applicators"){numGraphs = 3}
        for (let i = 0; i < numGraphs; i++){
            moduleData[`graph${i + 1}refPointLabel`].checkEntry(e.key);
        }
    }
})
