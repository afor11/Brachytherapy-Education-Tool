...
function tick(){
    ...
    if (module === "planar array of seeds"){
        ...
        moduleData.graph2.seeds.forEach((seed,ind) => {
            if ((seed.pos.x <= getMax(moduleData.graph2.xTicks)) && (seed.pos.x >= getMin(moduleData.graph2.xTicks))){
                ...
                ctx.beginPath();
                ctx.arc(screenPos.x,screenPos.y,0.08 * (moduleData.graph2.width / (getMax(moduleData.graph2.xTicks) - getMin(moduleData.graph2.xTicks))),0,2 * Math.PI);
                ctx.fill();
            }
        });
        ...
        if (canvas.width > (canvas.height * 0.9)){
            ...
            let text = "Seed Spacing: " + moduleData.graph1SeedSpacing.toFixed(2) + "U";
            ctx.font = getFontSize(moduleData.graph1AddSeeds.width * 0.8,moduleData.graph1AddSeeds.height,text) + "px monospace";
            ctx.fillText(
                text,
                moduleData.graph1ModelDropdown.button.width * 0.1,
    ...
    if ((module === "single seed") || (module === "string of seeds") || (module === "planar array of seeds")){
    ...
    if ((module === "single seed") || (module === "planar array of seeds")){
    ...
    if ((module === "string of seeds") || (module === "planar array of seeds")){
        ...
        moduleData.graph1.seeds.forEach((seed,ind) => {
            if ((seed.pos.x <= getMax(moduleData.graph1.xTicks)) && (seed.pos.x >= getMin(moduleData.graph1.xTicks))){
                ...
                ctx.beginPath();
                ctx.arc(screenPos.x,screenPos.y,0.08 * (moduleData.graph1.width / (getMax(moduleData.graph1.xTicks) - getMin(moduleData.graph1.xTicks))),0,2 * Math.PI);
                ctx.fill();
            }
        });
...
function initModule(mod,previousMod){
    ...
    if ((mod === "string of seeds") || (mod === "planar array of seeds")){
        // add options to the model dropdown
        for (let i = 0; i < 4; i++){
            let seedModel = [TheraSeed200,Best2301,GammaMedHDRPlus,BEBIG_GK60M21][i];
            moduleData.graph1ModelDropdown.options.push(new Button({
                x: 0, y: 0, width: 0, height: 0, bgColor: "white",label: {text: seedModel.name + " (" + seedModel.isotope + ")", font: "default", color: "black"}, outline: {color: "black", thickness: 0},
...
function adjustFormatting(){
    ...
    if (module === "planar array of seeds"){
        ...
        let graph1AirKermaSlider = moduleData.graph1AirKermaSlider;
        let graph1DwellTimeSlider = moduleData.graph1DwellTimeSlider;
        let addSeeds = moduleData.graph1AddSeeds;
        let removeSeeds = moduleData.graph1RemoveSeeds;
        ...
        if (canvas.width > canvas.height * 0.9){
            ...
            moduleData.graph1ModelDropdown.options.forEach((opt,ind) => {
                opt.x = 0;
                opt.y = graph1ModelDropdown.y + graph1ModelDropdown.height * (ind + 1);
                opt.width = graph1ModelDropdown.width;
                opt.height = graph1ModelDropdown.height;
            });
            ...
            addSeeds.x = 0;
            addSeeds.y = splitY + spaceAboveGraph * 0.1;
            addSeeds.width = graph1ModelDropdown.width;
            addSeeds.height = spaceAboveGraph * 0.2;

            removeSeeds.x = 0;
            removeSeeds.y = splitY + spaceAboveGraph * 0.4;
            removeSeeds.width = graph1ModelDropdown.width;
            removeSeeds.height = spaceAboveGraph * 0.2;
            ...
            addSeeds.x = 0;
            addSeeds.y = splitY + spaceToYSplit * 0.05;
            addSeeds.width = graph1ModelDropdown.width;
            addSeeds.height = spaceToYSplit * 0.1;

            removeSeeds.x = 0;
            removeSeeds.y = splitY + spaceToYSplit * 0.2;
            removeSeeds.width = graph1ModelDropdown.width;
            removeSeeds.height = spaceToYSplit * 0.1;
...
function checkSeedClicked(graph,boundingBox,graphName){
    let seedRadius = 0.16 * (graph.width / (getMax(graph.xTicks) - getMin(graph.xTicks))); // the radius is double the actual size of the drawn dots for ease of selection
    graph.seeds.forEach((seed,ind) => {
        ...
        if (Math.sqrt((mouse.x - seedScreenPos.x)**2 + (mouse.y - seedScreenPos.y)**2) <= seedRadius){
            ...
            if (module === "planar array of seeds"){
                moduleData.graph1AirKermaSlider.value = getValueFromAirKerma(seed);
                moduleData.graph1DwellTimeSlider.value = getValueFromDwellTime(seed);
            }
        }
    });
}
