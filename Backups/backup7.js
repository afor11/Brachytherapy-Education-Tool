    ...
    recalcDirection(){
        ...
        if (this.directionVec.z == 0) {
            if (this.directionVec.y == 0) {
                this.geometryRef = {x: 1, y: 1, z: (-this.directionVec.y -this.directionVec.z) / this.directionVec.x};
            }else{
                this.geometryRef = {x: 1, y: 1, z: (-this.directionVec.x -this.directionVec.z) / this.directionVec.y};
            }
        }else{
...
function tick(){
    ...
    if (moduleData.graph1){
        ...
        let initalZ;
        if (module === "brachytherapy applicators"){
            initalZ = moduleData.graph1.zSlice;
            moduleData.graph1.zSlice = moduleData.graph1refPoint.z; //fix reference point
        }else{
            moduleData.graph1refPoint.z = moduleData.graph1.zSlice; // move the reference point with camera
        }
        ...
        if (module === "brachytherapy applicators"){
            moduleData.graph1.zSlice = initalZ;
        }
    }
    if (moduleData.graph2){
        ...
        let initalZ;
        if (module === "brachytherapy applicators"){
            initalZ = moduleData.graph2.zSlice;
            moduleData.graph2.zSlice = moduleData.graph2refPoint.z; //fix reference point
        }else{
            moduleData.graph2refPoint.z = moduleData.graph2.zSlice; // move the reference point with camera
        }
        ...
        if (module === "brachytherapy applicators"){
            moduleData.graph2.zSlice = initalZ;
        }
        ...
        if (module != "single seed") {drawGraphSeeds(moduleData.graph2,moduleData.divBoundGraph2,"graph2");}
    }
...
function initModule(mod,previousMod){
    ...
    if (mod === "single seed"){
        ...
        moduleData = {
            graph1: (
                new Graph({x: 0, y: 0, width: 0, height: 0, seeds: [new Seed({x:0, y:0, z:0},{phi: 0, theta: 0},TheraSeed200,airKermaSliderLimits.LDR.min,0.00833)], xTicks: ticks, yTicks: ticks}) //x,y,width, and height are all set to 0 since they will be later formatted with an adjustFormatting() call
            ),
            divBoundGraph1: {bottom: 1157, height: 1057, left: 80, right: 1115, top: 100, width: 1035, x: 80, y: 100},
            graph2: (
                new Graph({x: 0, y: 0, width: 0, height: 0, seeds: [new Seed({x:0, y:0, z:0},{phi: 0, theta: 0},GammaMedHDRPlus,airKermaSliderLimits.HDR.min,0.00833)], xTicks: ticks, yTicks: ticks}) //x,y,width, and height are all set to 0 since they will be later formatted with an adjustFormatting() call
            ),
    ...
    if (mod === "string of seeds"){
        ...
        moduleData = {
            graph1: new Graph({x: 0, y: 0, width: 0, height: 0, seeds: [new Seed({x: 0, y: 0, z: 0},{phi: 0, theta: 0},BEBIG_GK60M21,airKermaSliderLimits.HDR.min,0.00833)],xTicks: xTicks, yTicks: yTicks}),
            divBoundGraph1: {bottom: 1157, height: 1057, left: 80, right: 1115, top: 100, width: 1035, x: 80, y: 100},
        ...
        moduleData.applicatorModelDropdown.options.forEach((button) => { // push second layer to module selection dropdown
            if (button.button.label == "Vaginal Cylinder"){
                for (let i = 30; i <= 60; i+= 10){
                    button.options.push(
                        new Button({x: 0, y: 0, width: 0, height: 0, bgColor: "white",label: {text: ("length: " + i / 10 + " cm"), font: "default", color: "black"}, outline: {color: "black", thickness: Math.min(canvas.width,canvas.height) * 0.001},
                            onClick: () => {
                                moduleData.applicatorModel = {name: "Vaginal Cylinder", length: i / 10, diameter: 2, angle: 90};
                        ...
                        for (let i = 20; i <= 30; i+= 5){
                            lengthSelection.options.push(
                                new Button({x: 0, y: 0, width: 0, height: 0, bgColor: "white",label: {text: ("Ovoids: " + i + " mm"), font: "default", color: "black"}, outline: {color: "black", thickness: Math.min(canvas.width,canvas.height) * 0.001},
                                    onClick: () => {
                                        moduleData.applicatorModel = {name: "Tandem/Ovoids", length: [2,3,4,5][lengthInd], diameter: 0.6, angle: [90,80,70,60][angleInd], ovoidDiameter: i / 10};
                                        moduleData.applicatorModelDropdown.collapseDropdown();
                                        adjustFormatting();
                                        render = true;
                                    }
                        ...
                        for (let i = 20; i <= 30; i+= 5){
                            lengthSelection.options.push(
                                new Button({x: 0, y: 0, width: 0, height: 0, bgColor: "white",label: {text: ("Ring Diameter: " + i + " mm"), font: "default", color: "black"}, outline: {color: "black", thickness: Math.min(canvas.width,canvas.height) * 0.001},
                                    onClick: () => {
                                        moduleData.applicatorModel = {name: "Tandem/Ring", length: [2,3,4,5][lengthInd], diameter: 0.6, angle: [90,80,70,60][angleInd], ringDiameter: i / 10};
                                        moduleData.applicatorModelDropdown.collapseDropdown();
                                        adjustFormatting();
                                        render = true;
                                    }
...
function adjustFormatting(){
    ...
    if (module === "brachytherapy applicators"){
        ...
        if (moduleData.applicatorModel.name === "Vaginal Cylinder"){
            applicatorModelDropdown.label = "Vaginal Cylinder: " + moduleData.applicatorModel.length * 10 + "mm length, " + moduleData.applicatorModel.diameter + "mm diameter";
        }
        if (moduleData.applicatorModel.name === "Tandem/Ovoids"){
            applicatorModelDropdown.label = "Tandem/Ovoids: " + moduleData.applicatorModel.length * 10 + "mm Tandem, " + moduleData.applicatorModel.angle + "°, " + moduleData.applicatorModel.ovoidDiameter + "mm Ovoids";
        }
        if (moduleData.applicatorModel.name === "Tandem/Ring"){
            applicatorModelDropdown.label = "Tandem/Ring: " + moduleData.applicatorModel.length * 10 + "mm Tandem, " + moduleData.applicatorModel.angle + "°, " + moduleData.applicatorModel.ringDiameter + "mm Ring";
        }
...
function drawGraphSeeds(graph,bound,name){
    ...
    graph.seeds.forEach((seed) => {
        if ((seed.pos.x <= getMax(graph.xTicks)) && (seed.pos.x >= getMin(graph.xTicks))
            && (seed.pos.y <= getMax(graph.yTicks)) && (seed.pos.y >= getMin(graph.yTicks))){
            let screenPos = graphToScreenPos({x: seed.pos.x,y: seed.pos.y},graph,bound);
            ctx.beginPath();
    ...
    if ((moduleData.selectedSeed != -1) && (moduleData.selectedGraph === name)){
        ctx.fillStyle = "rgb(169, 255, 103)";
        let screenPos = graphToScreenPos(graph.seeds[moduleData.selectedSeed].pos,graph,bound);
        ctx.beginPath();
        ctx.arc(screenPos.x,screenPos.y,seedRadius,0,2 * Math.PI);
        ctx.fill();
    }
...
function adjustApplicatorGraphFormat(graph,model,{refPoint: refPoint,drawFunction: drawFunction,boundingBox: boundingBox, view: view}){
    let seedModel = (moduleData.graph1.seeds.length > 0) ? moduleData.graph1.seeds[0].model : TheraSeed200;
    let airKerma = (seedModel.HDRsource ? airKermaSliderLimits.HDR.min : airKermaSliderLimits.LDR.min);
    if (model.name === "Vaginal Cylinder"){
        let adjustedSeeds = [];
        for (let i = model.length - 0.7; i >= 0; i -= 1){
            adjustedSeeds.push(new Seed({x: 0, y: i, z: 0},{phi: 0, theta: Math.PI / 2},seedModel,airKerma,0.00833));
        }
        graph.seeds = adjustedSeeds;
        let xTick = [];
        for (let i = -2; i <= 2; i += 0.125){xTick.push(i);}
        let yTick = [];
    ...
    if (model.name === "Tandem/Ovoids"){
        let adjustedSeeds = [];
        for (let i = model.length - 0.7; i >= 0; i -= 1){
            adjustedSeeds.push(new Seed({x: 0, y: i, z: 0},{phi: Math.PI / 2, theta: Math.PI / 2},seedModel,airKerma,0.00833));
        }
        for (let i = -1; i <= 1; i += 0.5){
            adjustedSeeds.push(new Seed({x: -model.ovoidDiameter / 2, y: -model.ovoidDiameter / 2, z: i},{phi: Math.PI / 2, theta: 0},seedModel,airKerma,0.00833));
            adjustedSeeds.push(new Seed({x: model.ovoidDiameter / 2, y: -model.ovoidDiameter / 2, z: i},{phi: Math.PI / 2, theta: 0},seedModel,airKerma,0.00833));
        }
        graph.seeds = adjustedSeeds;
        let xTick = [];
    ...
    if (model.name === "Tandem/Ring"){
        let adjustedSeeds = [];
        for (let i = model.length - 0.7; i >= 0; i -= 1){
            adjustedSeeds.push(new Seed({x: 0, y: i, z: 0},{phi: 0, theta: 0},seedModel,airKerma,0.00833));
        }
    ...
    if (view === "side"){
        // flip the seeds for the second graph
        graph.seeds.forEach((seed) => {
            seed.pos = {
                x: seed.pos.z,
                y: seed.pos.y,
            ...
            //seed.rot.phi += Math.PI / 2;
            //seed.recalcDirection();
        });
        moduleData[refPoint] = {
            x: moduleData[refPoint].z,
            y: moduleData[refPoint].y,
            z: moduleData[refPoint].x,
        };
    }
    if (view === "top"){
        // flip the seeds for the second graph
        graph.seeds.forEach((seed) => {
            ...
            //seed.rot.theta += Math.PI / 2;
            //seed.recalcDirection();
        });
        moduleData[refPoint] = {
            x: moduleData[refPoint].x,
            y: moduleData[refPoint].z,
            z: moduleData[refPoint].y,
        };
    }
    ...
    moduleData[drawFunction] = function () {
        ...
        if (model.name === "Tandem/Ovoids"){
            ...
            if (view === "side"){
                ...
                ctx.beginPath();
                ctx.roundRect(pos5.x,pos5.y,pos6.x - pos5.x,pos6.y - pos5.y,Math.min((pos6.x - pos5.x) / 2, (pos6.y - pos5.y) / 2));
                ctx.stroke();
                // draw tandem
                drawAngledTandem(model,graph,boundingBox,origin);
            }
