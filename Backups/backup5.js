...
class Seed {
    ...
    recalcDirection(){
        this.directionVec = {
            z: Math.cos(this.rot.phi),
            x: Math.sin(this.rot.phi) * Math.cos(this.rot.theta),
            y: Math.sin(this.rot.phi) * Math.sin(this.rot.theta)
        }
    }
}
class Graph {
    ...
    getPointDose(pos){
        return this.seeds.reduce((z,seed) => {
            ...
            return z + seed.calculateDose({
                ...
                theta: Math.acos((relativePos.x * seed.directionVec.x + relativePos.y * seed.directionVec.y + relativePos.z * seed.directionVec.z) / magnitude(relativePos))
            });
        },0)
    }
    getIsodose(refPoint){
        ...
        let refDose = this.getPointDose({x: refPoint.x, y: refPoint.y, z: this.zSlice});
        ...
        for (let i = getMin(this.yTicks); i <= getMax(this.yTicks); i+= dy){
            ...
            for (let j = getMin(this.xTicks); j <= getMax(this.xTicks); j+= dx){
                slice.push(100 * this.getPointDose({x: j, y: i, z: this.zSlice}) / refDose);
            }
            z.push(slice);
        }
        return z;
    ...
    checkDropdownClicked(){
        ...
        if (this.showing){
            this.options.forEach((button) => {
                if (button.options){
                    button.checkDropdownClicked();
                }else{
                    if (button.checkClicked()){ //.checkClicked() also calls the onClick() method on the button, and button.options is asked in case of nested dropdowns
                        this.showing = false;
                    }
                }
            });
...
function tick(){
    ...
    if (module === "planar array of seeds"){
        ...
        if (canvas.width > (canvas.height * 0.9)){
            ...
            ctx.font = getFontSize(moduleData.graph1AddSeeds.width * 0.8,moduleData.graph1AddSeeds.height,text) + "px monospace";
            ctx.fillText(
                text,
                moduleData.graph1ModelDropdown.button.width * 0.1,
        ...
        if (canvas.width > (canvas.height * 0.9)){
            ...
            ctx.font = getFontSize(moduleData.graph2ModelDropdown.width * 0.8,moduleData.graph2ModelDropdown.height,text) + "px monospace";
            ctx.fillText(
                text,
                moduleData.graph2ModelDropdown.button.x + moduleData.graph2ModelDropdown.button.width * 0.1,
        ...
        if ((moduleData.selectedGraph === "graph2") && (moduleData.selectedSeed != -1)){
            ...
            if (canvas.width > (canvas.height * 0.9)){
                ...
                ctx.font = getFontSize(moduleData.graph2AddSeeds.width * 0.8,moduleData.graph2AddSeeds.height,text) + "px monospace";
                ctx.fillText(
                    text,
    ...
    if (module === "brachytherapy applicators"){
        ...
        let treatmentTime;
        if (moduleData.graph1.seeds[0].model.HDRsource){
            treatmentTime = "Total Treatment Time: " + (Math.round((moduleData.graph1.seeds.reduce((acc,curr) => acc + curr.dwellTime, 0) * 60) * 100) / 100) + " mins";
        }else{
            treatmentTime = "Permanent Implant";
        }
        ctx.font = getFontSize(canvas.width / 3,canvas.height * 0.05,treatmentTime) + "px monospace";
        ctx.fillStyle = "black";
        ctx.beginPath();
        ...
        adjustApplicatorRenderer(moduleData.graph1,moduleData.applicatorModel,{refPoint: "graph1refPoint", drawFunction: "graph1DrawApplicator", boundingBox: moduleData.divBoundGraph1});
        adjustApplicatorRenderer(moduleData.graph2,moduleData.applicatorModel,{refPoint: "graph2refPoint", drawFunction: "graph2DrawApplicator", boundingBox: moduleData.divBoundGraph2});
    ...
    if ((module === "string of seeds") || (module === "planar array of seeds") || (module === "brachytherapy applicators")){
        ...
        moduleData.graph1.seeds.forEach((seed,ind) => {
            if ((seed.pos.x <= getMax(moduleData.graph1.xTicks)) && (seed.pos.x >= getMin(moduleData.graph1.xTicks))){
                if ((moduleData.selectedGraph === "graph1") && (moduleData.selectedSeed == ind)){
    ...
    if (moduleData.graph1){
        ...
        ctx.fill();
        moduleData.graph1refPoint.z = moduleData.graph1.zSlice;
    ...
    if (moduleData.graph2){
        ...
        ctx.fill();
        moduleData.graph2refPoint.z = moduleData.graph2.zSlice;
        ...
        ctx.lineWidth = 0;
        seedRadius = Math.min(canvas.width,canvas.height * 0.9) * 0.005;
        moduleData.graph2.seeds.forEach((seed,ind) => {
            if ((seed.pos.x <= getMax(moduleData.graph2.xTicks)) && (seed.pos.x >= getMin(moduleData.graph2.xTicks))){
                if ((moduleData.selectedGraph === "graph2") && (moduleData.selectedSeed == ind)){
                    ctx.fillStyle = "rgb(169, 255, 103)";
                }else{
                    ctx.fillStyle = "rgb(0, 0, 0)";
                }
                let screenPos = graphToScreenPos({x: seed.pos.x,y: seed.pos.y},moduleData.graph2,moduleData.divBoundGraph2);
                ctx.beginPath();
                ctx.arc(screenPos.x,screenPos.y,seedRadius,0,2 * Math.PI);
                ctx.fill();
            }
    ...
    if ((module === "planar array of seeds") || (module === "brachytherapy applicators")){
        //draw "dwell positions of seeds" graph2
        ctx.lineWidth = 0;
        let seedRadius = Math.min(canvas.width,canvas.height * 0.9) * 0.005;
        moduleData.graph2.seeds.forEach((seed,ind) => {
...
function initModule(mod,previousMod){
    ...
    if (mod === "single seed"){
        ...
        moduleData = {
            graph1: (
                new Graph({x: 0, y: 0, width: 0, height: 0, seeds: [new Seed({x:0, y:0, z:0},{phi: Math.PI / 2, theta: 0},TheraSeed200,airKermaSliderLimits.LDR.min,0.00833)], xTicks: ticks, yTicks: ticks}) //x,y,width, and height are all set to 0 since they will be later formatted with an adjustFormatting() call
            ),
            divBoundGraph1: {bottom: 1157, height: 1057, left: 80, right: 1115, top: 100, width: 1035, x: 80, y: 100},
            graph2: (
                new Graph({x: 0, y: 0, width: 0, height: 0, seeds: [new Seed({x:0, y:0, z:0},{phi: Math.PI / 2, theta: 0},GammaMedHDRPlus,airKermaSliderLimits.HDR.min,0.00833)], xTicks: ticks, yTicks: ticks}) //x,y,width, and height are all set to 0 since they will be later formatted with an adjustFormatting() call
            ),
    ...
    if (mod === "string of seeds"){
        ...
        moduleData = {
            graph1: new Graph({x: 0, y: 0, width: 0, height: 0, seeds: [new Seed({x: 0, y: 0, z: 0},{phi: Math.PI / 2, theta: 0},BEBIG_GK60M21,airKermaSliderLimits.HDR.min,0.00833)],xTicks: xTicks, yTicks: yTicks}),
            divBoundGraph1: {bottom: 1157, height: 1057, left: 80, right: 1115, top: 100, width: 1035, x: 80, y: 100},
    ...
    if (mod === "planar array of seeds"){
        ...
        moduleData = {
            graph1: new Graph({x: 0, y: 0, width: 0, height: 0, seeds: [],xTicks: tick, yTicks: tick}),
            divBoundGraph1: {bottom: 1157, height: 1057, left: 80, right: 1115, top: 100, width: 1035, x: 80, y: 100},
            ...
            graph2: new Graph({x: 0, y: 0, width: 0, height: 0, seeds: [],xTicks: tick, yTicks: tick}),
            divBoundGraph2: {bottom: 1157, height: 1057, left: 80, right: 1115, top: 100, width: 1035, x: 80, y: 100},
        ...
        for (let i = -1.5; i <= 1.5; i += 1){
            for (let j = -1.5; j <= 1.55; j += 1){
                moduleData.graph1.seeds.push(new Seed({x: j, y: i, z: 0},{phi: Math.PI / 2, theta: 0},TheraSeed200,airKermaSliderLimits.LDR.min,0.00833));
                moduleData.graph1.geometryRef = moduleData.graph1refPoint;
                moduleData.graph2.seeds.push(new Seed({x: j, y: i, z: 0},{phi: Math.PI / 2, theta: 0},GammaMedHDRPlus,airKermaSliderLimits.HDR.min,0.00833));
    ...
    if (mod === "brachytherapy applicators"){
        moduleData = {
            graph1: new Graph({x: 0, y: 0, width: 0, height: 0, seeds: [],xTicks: [], yTicks: []}),
            divBoundGraph1: {bottom: 1157, height: 1057, left: 80, right: 1115, top: 100, width: 1035, x: 80, y: 100},
            graph1refPoint: {x: 0, y: 0, z: 0},
            ...
            graphAirKermaSlider: new Slider(0,0,0,0,"black",0,0.5),
            ...
            resetDwellTimes: new Button({
                ...
                onClick: () => {
                    ...
                    moduleData.graph2.seeds.forEach((seed) => {seed.dwellTime = 0.00833});
                },
        ...
        moduleData.applicatorModel = {name: "Vaginal Cylinder", length: 40, diameter: 0.6, angle: 90};
        adjustApplicatorGraphFormat(moduleData.graph1,moduleData.applicatorModel,{refPoint: "graph1refPoint", drawFunction: "graph1DrawApplicator", boundingBox: moduleData.divBoundGraph1});
        adjustApplicatorGraphFormat(moduleData.graph2,moduleData.applicatorModel,{refPoint: "graph2refPoint", drawFunction: "graph2DrawApplicator", boundingBox: moduleData.divBoundGraph2});
        ...
        for (let i = 0; i < 3; i ++){
            moduleData.applicatorModelDropdown.options.push(
                new Dropdown(
                    new Button({
                        x: 0, y: 0, width: 0, height: 0, bgColor: "white",label: {text: (["Vaginal Cylinder","Tandem/Ovoids","Tandem/Ring"][i]), font: "default", color: "black"}, outline: {color: "black", thickness: Math.min(canvas.width,canvas.height) * 0.001},
                        onClick: () => {
                            moduleData.applicatorModel.name = ["Vaginal Cylinder","Tandem/Ovoids","Tandem/Ring"][i];
                        }
                    }),
                    []
                ),
        ...
        moduleData.applicatorModelDropdown.options.forEach((button) => {
            for (let i = 90; i >= 60; i-= 10){
                button.options.push(
                    new Dropdown( new Button({x: 0, y: 0, width: 0, height: 0, bgColor: "white",label: {text: ("Angle: " + i + "Deg"), font: "default", color: "black"}, outline: {color: "black", thickness: Math.min(canvas.width,canvas.height) * 0.001},onClick: () => {}}),[]),
                );
            }
        });
    }
            ...
            moduleData.graph1ModelDropdown.options.push(new Button({
                ...
                onClick: () => {
                    ...
                    if (mod === "string of seeds"){
                        ...
                        moduleData.dwellTimeSlider.value = selectedSeed ? getValueFromDwellTime(selectedSeed) : 0.00833;
                    }else{
                        moduleData.graph1AirKermaSlider.value = selectedSeed ? getValueFromAirKerma(selectedSeed) : 0;
                        moduleData.graph1DwellTimeSlider.value = selectedSeed ? getValueFromDwellTime(selectedSeed) : 0.00833;
                    }
            ...
            moduleData.graph2ModelDropdown.options.push(new Button({
                ...
                onClick: () => {
                    ...
                    if (mod === "string of seeds"){
                        ...
                        moduleData.graph2DwellTimeSlider.value = selectedSeed ? getValueFromDwellTime(selectedSeed) : 0.00833;
                    }
...
function adjustFormatting(){
    ...
    if (module === "brachytherapy applicators"){
        ...
        let graphAirKermaSlider = moduleData.graphAirKermaSlider;
        let graphDwellTimeSlider = moduleData.graphDwellTimeSlider;

        if ((canvas.width / (canvas.height * 0.9)) > 1.2){
            graph1.width = canvas.width / 3;
            graph1.height = canvas.height * 0.9;
            graph1.x = canvas.width / 3;
            graph1.y = canvas.height * 0.1 + ((canvas.height * 0.9) - graph1.height) / 2;

            graph2.width = canvas.width / 3;
            graph2.height = canvas.height * 0.9;
            graph2.x = 2 * canvas.width / 3;
            graph2.y = canvas.height * 0.1 + ((canvas.height * 0.9) - graph2.height) / 2;

            resetDwellTimes.width = canvas.width / 3;
            ...
            graphModelDropdown.width = canvas.width / 3;
            ...
            applicatorModelDropdown.width = canvas.width / 3;
            applicatorModelDropdown.height = canvas.height * 0.05; //label is reolved at the bottom of this if statement
        }else{
            graph1.width = canvas.width / 2;
            graph1.height = canvas.height * 0.75;
            graph1.x = 0;
            graph1.y = canvas.height * 0.25;

            graph2.width = canvas.width / 2;
            graph2.height = canvas.height * 0.75;
            graph2.x = canvas.width / 2;
            graph2.y = canvas.height * 0.25;
            ...
            graphModelDropdown.x = 0.4 * canvas.width;
        ...
        if (moduleData.applicatorModel.name === "Vaginal Cylinder"){
            applicatorModelDropdown.label = "Vaginal Cylinder: " + moduleData.applicatorModel.length + "mm length, " + moduleData.applicatorModel.diameter + "mm diameter";
        }
        if (moduleData.applicatorModel.name === "Tandem/Ovoids"){
            applicatorModelDropdown.label = "Tandem/Ovoids: " + moduleData.applicatorModel.length + "mm Tandem, " + moduleData.applicatorModel.angle + "°, " + moduleData.applicatorModel.ovoidDiameter + "mm Ovoids";
        }
        if (moduleData.applicatorModel.name === "Tandem/Ring"){
            applicatorModelDropdown.label = "Tandem/Ring: " + moduleData.applicatorModel.length + "mm Tandem, " + moduleData.applicatorModel.angle + "°, " + moduleData.applicatorModel.ringDiameter + "mm ring";
        }
        ...
        let fontSizes = [];
        moduleData.applicatorModelDropdown.options.forEach(({button: button}) => fontSizes.push(getFontSize(button.width,button.height,button.label)));
        moduleData.applicatorModelDropdown.options.forEach(({button: opt}) => {opt.font = getMin(fontSizes) + "px monospace";});

        alignApplicatorTypeDropdown(moduleData.applicatorModelDropdown,0);
        ...
        fontSizes = [];
        moduleData.graphModelDropdown.options.forEach((button) => fontSizes.push(getFontSize(button.width,button.height,button.label)));
...
addEventListener("keydown",function (e) {
    if (e.key === "ArrowDown"){
        if (moduleData.graph1){
            moduleData.graph1.zSlice = Math.max(moduleData.graph1.zSlice - 0.05,Math.max(getMin(moduleData.graph1.xTicks),getMin(moduleData.graph1.yTicks)));
            render = true;
        }
        if (moduleData.graph2){
            moduleData.graph2.zSlice = Math.max(moduleData.graph2.zSlice - 0.05,Math.max(getMin(moduleData.graph2.xTicks),getMin(moduleData.graph2.yTicks)));
            render = true;
        }
    }
    if (e.key === "ArrowUp"){
        if (moduleData.graph1){
            moduleData.graph1.zSlice = Math.min(moduleData.graph1.zSlice + 0.05,Math.min(getMax(moduleData.graph1.xTicks),getMax(moduleData.graph1.yTicks)));
            render = true;
        }
        if (moduleData.graph2){
            moduleData.graph2.zSlice = Math.min(moduleData.graph2.zSlice + 0.05,Math.min(getMax(moduleData.graph2.xTicks),getMax(moduleData.graph2.yTicks)));
            render = true;
        }
    }
})
...
function getDwellTimeFromSlider(slider){
    return slider.value * 0.16666; // slider from 30 seconds to 10 minutes
}
...
function adjustApplicatorGraphFormat(graph,model,{refPoint: refPoint,drawFunction: drawFunction,boundingBox: boundingBox}){
    if (model.name === "Vaginal Cylinder"){
        let adjustedSeeds = [];
        for (let i = model.length - 6; i >= 0; i-= 2){
            adjustedSeeds.push(new Seed({x: 0, y: i, z: 0},{phi: 0, theta: 0},TheraSeed200,airKermaSliderLimits.LDR.min,0.00833));
        }
        graph.seeds = adjustedSeeds;
        let xTick = [];
        for (let i = -2; i <= 2; i += 0.25){xTick.push(i);}
        let yTick = [];
        for (let i = -2; i <= model.length + 2; i += 0.25){yTick.push(i);}
        graph.xTicks = xTick;
        graph.yTicks = yTick;
        moduleData[refPoint] = {x: model.diameter / 2 + 0.5, y: 0, z: 0};
        adjustApplicatorRenderer(graph,model,{drawFunction: drawFunction,boundingBox: boundingBox});
    }
}
function adjustApplicatorRenderer(graph,model,{drawFunction: drawFunction,boundingBox: boundingBox}){
    if (model.name === "Vaginal Cylinder"){
        moduleData[drawFunction] = function () {
            ctx.strokeStyle = "black";
            ctx.lineWidth = Math.min(canvas.width,canvas.height * 0.9) * 0.005;
            let pos1 = graphToScreenPos({x: -model.diameter / 2, y: model.length},graph,boundingBox);
            let pos2 = graphToScreenPos({x: model.diameter / 2, y: -2},graph,boundingBox);
            ctx.beginPath();
            ctx.roundRect(pos1.x,pos1.y,pos2.x - pos1.x, pos2.y - pos1.y, [(pos2.x - pos1.x) / 2,(pos2.x - pos1.x) / 2,0,0]);
            ctx.stroke();
        }
        moduleData.graph2.seeds = moduleData.graph1.seeds;
        moduleData.graph2.seeds.forEach((seed,ind) => {
            moduleData.graph1.seeds[ind].y = seed.z;
            moduleData.graph1.seeds[ind].z = seed.y;
        });
    }
}
function alignApplicatorTypeDropdown(dropdown,depth){
    dropdown.options.forEach((opt,ind) => {
        if (opt.options){
            ...
            alignApplicatorTypeDropdown(opt,depth + 1);
        }else{
    ...
    dropdown.options.forEach((child) => {
        if (child.button){
            fontSizes.push(getFontSize(child.button.width,child.button.height,child.button.label));
        }else{
            fontSizes.push(getFontSize(child.width,child.height,child.label));
        }
    });
    dropdown.options.forEach((child) => {
        if (child.button){
            child.button.font = getMin(fontSizes) + "px monospace";
        }else{
            child.font = getMin(fontSizes) + "px monospace";
        }
    });
}